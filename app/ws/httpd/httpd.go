...
func (c *Client) handlerHealth(w http.ResponseWriter, r *http.Request) {
	if err := network.CheckHTTPhost(r.RemoteAddr, `^((127\.0\.0\.)|(10\.)|(192\.168\.)|(172\.1[6-9]\.)|(172\.2[0-9]\.)|(172\.3[0-1]\.))`); err != nil {
		c.log.Sugar().Errorw("remote address test failed", "service_id", c.serviceID, "path", r.URL.Path, "error", err.Error(), "remote_addr", r.RemoteAddr)
		http.Error(w, "403 Forbidden", http.StatusForbidden)
		return
	}

	testCB := couchbase.NewClient(c.cbPresence)
	if err := testCB.Test(); err != nil {
		c.log.Sugar().Errorw("couchbase test failed", "service_id", c.serviceID, "path", r.URL.Path, "error", err.Error(), "remote_addr", r.RemoteAddr)
	...
	if err != nil {
		c.log.Sugar().Errorw("rabbitmq test failed", "service_id", c.serviceID, "path", r.URL.Path, "error", err.Error(), "remote_addr", r.RemoteAddr)
		http.Error(w, "503 Service Unavailable", http.StatusServiceUnavailable)
		return
	}
