...
func (c *Client) handlerHealth(w http.ResponseWriter, r *http.Request) {
	...
	testSQLman := mariadb.NewClient(c.sqlMan)
	if err := testSQLman.Test(); err != nil {
		c.log.Sugar().Errorw("sql manager test failed", "path", r.URL.Path, "error", err.Error(), "remote_addr", r.RemoteAddr)
		http.Error(w, "503 Service Unavailable", http.StatusServiceUnavailable)
		return
	}

	testSQLrw := mariadb.NewClient(c.sqlRW)
	if err := testSQLrw.Test(); err != nil {
		c.log.Sugar().Errorw("sql rw test failed", "path", r.URL.Path, "error", err.Error(), "remote_addr", r.RemoteAddr)
		http.Error(w, "503 Service Unavailable", http.StatusServiceUnavailable)
		return
	}
